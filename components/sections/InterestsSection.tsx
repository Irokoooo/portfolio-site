...
const academicInterests = [
  ...
  },
  {
    ...
    questions: [
      [
        { q: { zh: "Agent 何时会从工具调用者变成真正的协作者？", en: "When does an agent become a collaborator rather than a tool caller?" }, thought: { zh: "关键不只是自治，而是能否理解目标、汇报不确定性并接受纠偏。", en: "The key is not autonomy alone, but understanding goals, surfacing uncertainty, and accepting correction." } },
        { q: { zh: "多 Agent 并行时，怎样避免冲突与重复劳动？", en: "How can parallel agents avoid conflicts and duplicated work?" }, thought: { zh: "清晰边界、共享状态与可追溯交接，比增加 Agent 数量更重要。", en: "Clear boundaries, shared state, and traceable handoffs matter more than agent count." } },
        { q: { zh: "长期记忆应该属于模型、Agent 还是用户？", en: "Should long-term memory belong to the model, the agent, or the user?" }, thought: { zh: "我更倾向于用户可见、可编辑、可迁移的记忆层。", en: "I favor a user-visible, editable, and portable memory layer." } },
        { q: { zh: "Agent 的可靠性应该如何评估？", en: "How should agent reliability be evaluated?" }, thought: { zh: "不只看最终答案，还要看过程、恢复能力与失败边界。", en: "Not just final answers, but process, recovery, and failure boundaries." } },
      ],
      [
  ...
  {
    id: "psychology",
    title: { zh: "应用神经营销学与脑神经科学", en: "Applied Neuromarketing & Neuroscience" },
    desc: {
      zh: "关注注意、情绪、记忆与奖赏系统如何塑造消费者感知，并思考脑科学在产品与品牌研究中的应用边界。",
      en: "Studying how attention, emotion, memory, and reward systems shape consumer perception, while examining the ethical limits of neuroscience in product and brand research.",
    },
    questions: [
      [
        { q: { zh: "注意力被捕获和真正产生兴趣有什么区别？", en: "What separates captured attention from genuine interest?" }, thought: { zh: "前者可能只是一瞬的刺激，后者需要意义、记忆与动机共同参与。", en: "The first may be momentary stimulus; the second requires meaning, memory, and motivation." } },
        { q: { zh: "品牌记忆是如何在大脑中形成的？", en: "How does brand memory form in the brain?" }, thought: { zh: "重复并不够，还需要情绪唤起和稳定的联想网络。", en: "Repetition is not enough; emotional arousal and stable associations also matter." } },
        { q: { zh: "奖赏预测误差如何影响消费选择？", en: "How does reward prediction error influence consumer choice?" }, thought: { zh: "超出预期的体验更容易被编码，但持续透支惊喜会削弱信任。", en: "Positive surprise is memorable, but overusing it can erode trust." } },
        { q: { zh: "感官线索如何改变产品价值感知？", en: "How do sensory cues alter perceived product value?" }, thought: { zh: "颜色、声音、触感与气味都可能在意识判断之前完成预设。", en: "Color, sound, touch, and scent can frame value before conscious judgment." } },
      ],
      [
        { q: { zh: "情绪唤起越强，广告效果就越好吗？", en: "Does stronger emotional arousal always improve advertising?" }, thought: { zh: "不一定；情绪必须与品牌和行动线索建立连接。", en: "Not always; emotion must connect to the brand and intended action." } },
        { q: { zh: "眼动数据能否代表真实偏好？", en: "Can eye-tracking data represent true preference?" }, thought: { zh: "它能说明看见了什么，却不能单独解释为什么喜欢。", en: "It shows what was seen, but cannot alone explain why it was liked." } },
        { q: { zh: "价格为什么会引发类似疼痛的神经反应？", en: "Why can prices trigger pain-like neural responses?" }, thought: { zh: "支付意味着放弃其他可能，损失与价值判断在同一刻发生。", en: "Payment means giving up alternatives; loss and value are assessed together." } },
        { q: { zh: "神经指标与自我报告冲突时该相信谁？", en: "What should we trust when neural signals and self-report conflict?" }, thought: { zh: "都不能单独成为答案，最好结合行为结果进行三角验证。", en: "Neither alone; triangulate both against actual behavior." } },
      ],
      [
        { q: { zh: "神经营销学如何避免沦为“读心术”叙事？", en: "How can neuromarketing avoid mind-reading narratives?" }, thought: { zh: "明确概率解释、研究限制与可重复性，不把相关性包装成因果。", en: "Be explicit about probability, limitations, and replication; never sell correlation as causation." } },
        { q: { zh: "个性化营销的神经数据边界在哪里？", en: "Where is the boundary for neural data in personalization?" }, thought: { zh: "知情同意、数据最小化和退出权应该先于转化率。", en: "Consent, minimization, and the right to opt out should come before conversion." } },
        { q: { zh: "产品体验如何降低认知负荷？", en: "How can product experiences reduce cognitive load?" }, thought: { zh: "减少竞争信号，强化层级，并把复杂度留给系统而不是用户。", en: "Reduce competing signals, strengthen hierarchy, and keep complexity in the system." } },
        { q: { zh: "脑科学能为品牌长期关系提供什么？", en: "What can neuroscience add to long-term brand relationships?" }, thought: { zh: "帮助理解习惯、信任和记忆，而不是只追逐即时点击。", en: "It helps explain habit, trust, and memory rather than only chasing clicks." } },
      ],
    ],
  },
  {
