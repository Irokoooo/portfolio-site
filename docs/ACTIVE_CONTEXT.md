...
## 2026-09-04｜内容修复与加载优化（本轮）

- About 树生长动画仅在进入栏目时播放一次，离开暂停，重新进入从头播放。
- 黑客松移至学术与实践左栏，Coding Lady、SheNicest、EAZO、欧莱雅均可展开；Coding Lady 详情复用现场照片。
- 新增考文纽斯暑期经历（桌面与移动端）；英文校名按官网使用 Corvinus University of Budapest，个人经历日期沿用简历。足迹点亮匈牙利。
- 字节状态改为至今，增加适合小尺寸显示的标志；修正函数猫/小民快跑指标中的弯引号，避免 NaN。
- Vibe 项目标题、摘要、工作案例正文与结果支持英文；Zenow/SpellBook 在线入口前置。
- 工作案例封面使用静态缩略图，动画仅在详情加载。大型导航/奖项 SVG 内嵌位图压缩，保持原布局。
- 修复首页桌面/手机同时挂载导致的资源重复加载、深链接双重弹窗；现在按屏幕宽度仅挂载一个版本。
- 下一站暂不整体改版，仅删除用户指定句子；待选择视觉方向。生产环境速度与动图深度压缩仍需后续实测。


## 2026-09-01｜新增工作系统作品集

- 将原先隐藏的 `Business Analysis` 子入口重新定位并启用为 `Work Systems / 工作系统`，用于展示真实工作中的 AI 工作流、内部工具与规模化业务系统；桌面端与移动端入口已同步开启。
- 新增 `AI Evaluation Workbench｜可信评测基础设施`：一期 63 位专家 / 310 道候选题 → 170 道高价值题，Skill 抽检一致率约 50% → 80%，实际运行 500 题级批处理与 1,000 题级流式任务，交接后被 3 个团队使用。内容明确区分个人设计实现、团队协作与 leader 主导部分。
- 新增 `多语种质检工作台－飞书边栏插件`：约 12 名内部 QC 使用约 1.5 个月、覆盖约 2 期生产任务，单条 QC 由约 10 分钟降至 3–5 分钟；从问题识别、产品设计、Vibe Coding、上线、说明书到迭代由用户独立推进。
- 已接入 4 个 Evaluation Workbench 和 3 个多语种质检工作台流程动图。原始 GIF 共约 72 MB，已转为约 29 MB 的动画 WebP；详情页使用单项按需加载轮播，避免一次下载全部素材。

---

> 最后更新：2026-08-16（第九十二轮规划）
...
本轮先完成内容梳理与信息架构，不直接修改正式页面。开发时统一在
`codex/zenow-spellbook-growth-galaxy` 功能分支进行，验收后再合并回 `main`。

### 主线 A：Vibe Coding 作品集新增 Zenow 与 Spell Book

两个项目均接入 `Side Works → Vibe Coding`，沿用现有展示形式：

1. 作品卡片：封面、标题、一句话价值、时间、类型与 3–5 个标签。
2. 详情抽屉：封面或演示媒体，以及“需求觉察 → 痛点与挑战 → 落地解法 → 迭代心得”的正文。
3. 有公开地址时补充 GitHub 与在线 Demo；没有公开地址时不展示空按钮。
4. 同时准备中文和英文 Markdown，保持全站双语一致。

#### 已知信息

- Spell Book 已在 `components/sections/ProjectsSection.tsx` 留有基础资料：OpenClaw 可视化智能工作台，
  Web + 本地 Agent 双端架构，支持离线任务队列、流式输出和 Windows 使用；GitHub 为
  `https://github.com/Irokoooo/spellbook-openclaw`。
- Zenow 项目已在本机 `D:\Coding\Zenow` 完成只读盘点。它是一款以地图世界为载体的 AI 健康陪伴游戏：
  用户完成现实任务获得骰子，在等距小镇中移动、触发事件和建设，并通过成长花瓣、技能牌、
  每日花语牌、记忆银河与 AI 对话，把现实成长映射为可见的世界变化。
- Zenow 的核心价值主张可概括为：“我做的事情被看见了，我的世界因此发生了变化，Zenow 也真的更了解我了一点。”
- 当前本机版本已形成可玩的技术 MVP，已有任务 → 骰子 → 移动 → 事件/奖励 → 建筑/商店闭环；
  另有问卷、Supabase 云端基础、AI 小屋、技能牌、成长花瓣、随机事件 V2、每日花语牌和记忆银河。
- 可验证工程信息：原生 HTML/CSS/JavaScript Modules、Canvas、Three.js、Supabase、Capacitor 8；
  仓库为 `https://github.com/Irokoooo/Zenow.git`。状态文档记录 340/340 自动化测试通过。
  用户已确认 `https://zen0w.me/?ui=20260725-2` 为当前正式部署最新版；作品集可直接链接该地址。
- Zenow 参赛活动正式名称为“她来创造 · Coding Lady”女性 AI 创造者黑客松，团队从比赛“五种时间”命题中选择“心流时间”。
- 用户负责产品的全部技术落地，并以一个 Agent 作为中台协调 Codex、Claude Code 与多个并行 Agent，在限定边界内并行交付、避免代码冲突。
- 已验证结果：50+ 人试玩社群、黑客松创意奖、完成现场 coding 与线下路演。
- 作品集展示使用三张有效现场照片做横向循环画廊；活动介绍图和评审规则图只作信息来源、不上站。两次补发的“第一张”文件均为 1×1 空 PNG，等待用户之后补充有效原图。
- 现有 QA 截图已覆盖登录、Onboarding、任务书、任务完成奖励、3D 骰子、每日花语牌和记忆银河；
  后续视频应优先补充连续操作、动效、地图移动、事件选择、AI 对话与奖励反馈。

#### 每个项目需要准备的素材

**P0：可先交给 Agent 阅读并产出初稿**

- PRD、需求说明、比赛说明或路演稿。
- GitHub 仓库地址；如果是私有仓库，可提供 README、目录树或脱敏后的核心文件。
- 项目时间、团队人数、个人角色和实际负责范围。
- 一句话说明：服务谁、解决什么问题、最终做出了什么。
- 3–6 张关键截图，至少覆盖首页、核心流程和结果页。
- 当前结果：是否完成 Demo、是否参赛/获奖、用户反馈、测试数据或其他可验证成果。

**P1：用于提升作品集说服力**

- 30–90 秒演示视频或 GIF。
- 16:9 / 4:3 高质量封面图；文件名使用英文 slug。
- 架构图、用户流程图、关键数据模型或 Agent 工作流图。
- 最困难的 2–3 个问题、失败尝试、转折点和最终解法。
- 开发工具链、模型、框架、部署方式以及为什么这样选择。
- 版本迭代记录：初版问题、修改动作和迭代后差异。
- 在线 Demo、GitHub、比赛页面或其他公开证明链接。
...
- 团队协作方式、个人决策与边界。
- 如果重做一次会改变什么。
- 对产品、AI Agent、Vibe Coding 或黑客松协作的新认识。

#### 内容整理流程

1. 作者先提供现有 PRD、README、仓库与媒体，不需要预先写成长文。
2. Agent 读取材料后，先生成“已确认事实 / 合理推断 / 信息缺口”三栏摘要。
3. Agent 只针对无法从材料确认的关键缺口进行第二轮追问。
4. 作者确认事实后，生成中英文项目卡片与详情 Markdown 初稿。
5. 接入页面并完成桌面端、移动端、双语和媒体加载验证。

### 主线 B：成长星河节点与串联逻辑头脑风暴

目标不是先堆视觉效果，而是先确定“什么值得成为节点，以及节点之间为什么相连”。

#### 建议的节点层级

- **主节点 / 里程碑**：实习、比赛、交换、重要项目、证书、阶段性选择。
- **作品节点**：Zenow、Spell Book、Luma Flow、LawFlaw、个人网站等可验证产出。
- **能力节点**：产品设计、商业分析、数据分析、Agent 编排、多语言与跨文化沟通。
- **学习节点**：课程、语言进度、阅读主题、工具实验和阶段性认知升级。
- **微节点 / 碎片**：一次失败、灵感、对话、短期观察或当天值得记录的事件。

#### 建议的连线语义

- `启发`：前一个经历产生了后一个想法。
- `能力迁移`：某次学习或实习能力被应用到新项目。
- `共同主题`：不同节点都围绕同一长期兴趣。
- `迭代`：同一产品或方法的版本演进。
- `合作`：因同一团队、比赛或伙伴形成关联。
- `转折`：失败、反馈或外部事件改变了下一步方向。

#### 本轮需要共同决定的问题

1. 成长星河的主轴更偏“按时间成长”，还是“按主题聚类，时间作为辅助”？
2. 节点以客观事件为主，还是允许大量主观想法和碎碎念？
3. Zenow、Spell Book 等作品是独立大节点，还是挂在对应能力与经历下面？
4. 连线由作者手动定义，还是系统依据标签、时间和引用关系自动推荐？
5. 点击节点后展示短卡片、完整 Markdown，还是进一步跳转到作品集详情？
6. 哪些内容可公开，哪些只适合本地保存或匿名化展示？

#### 推荐的首版范围

- 先选 12–20 个真实节点，覆盖 3–5 年关键成长路径。
- 每个节点只要求：日期、标题、类型、一句话、2–4 个标签和公开级别。
- 先手动定义 15–30 条有明确语义的连线，验证叙事是否自然。
- 结构确认后再决定使用 2D 编辑器映射到 3D，或直接开发时间型 3D 星河。
...
## 2026-09-01 作品集信息架构更新

### 本轮已完成

- Vibe Coding 统一承载全部代码作品，页内分为「日常项目」与「工作项目」，不再单独展示 Work Systems 导航。
- 补回 SpellBook 项目，并为 Zenow 更换为真实产品入口页封面。
- 字节跳动工作经历增加 Proof of Work，可直达 AI 评测工作台与多语种质检工作台案例。
- Academic & Practice 新增「黑客松项目」分栏，Coding Lady / Zenow 从社会实践迁入该分栏。
- 兴趣方向更新：
  - 数字化转型 → 人工智能发展与 Agent 前沿
  - 经济心理学与行为决策 → 应用神经营销学与脑神经科学
- 字节跳动经历已更新为正式版本，补充 Evaluation Workbench、LightFlow、模型实验、5 万+ 数据交付与 2,000+ 专家运营等成果。
- Career 工作卡片改为严格单语渲染：中文模式只显示中文正文与界面标签，英文模式只显示英文内容。
- About Me 已替换为新的中英文探索叙事，强调碳基 / 硅基智能边界、产品创造与「把复杂留给系统，把简单留给人」。
- Vibe Coding 右侧封面流新增拼豆工具与个人学习网络 Demo 两张作品图。
- AI 兴趣方向进一步聚焦 Agent Harness、多智能体协作与具身智能前沿。
- 岭南大学教育经历新增 3 张校园 / 商学院照片，并复用经历详情图片滚动栏。
- 荣誉时间轴新增 2026.05 数字媒体科技作品竞赛北京赛区三等奖、2026.06 三创赛烽智安新队、2026.07 品牌策划全国总决赛二等奖；移除 2026.03 农里鸿项目旧条目。

## 2026-09-03 内容可靠性与性能修复

- Vibe Coding 取消分类切换，改为同页展示：工作项目在前、日常项目在后。
- 16 份中英文项目 Markdown 同步为公开静态资源，由 CDN 缓存读取，避免动态 API 失败导致正文打不开。
- 为个人作品集早期版本、大学生综测系统、APPLE ASSENCE 补充抽屉内项目截图。
- 教育 / 经历图片改用 Next Image 按尺寸优化，减少原始大图传输，并保留懒加载行为。
- About Me 树生长视频恢复循环播放。

### 待补充内容

- 欧莱雅黑客松已作为社会实践简卡展示：项目 Beauty Copilot，状态为进行时。
- EAZO 数字艺术黑客松已作为社会实践简卡展示：2026.09.05—09.06，最佳世界观奖。
- SpellBook 后续可补一张代表性产品首图或演示视频。
